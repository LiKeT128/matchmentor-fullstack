...
class AdvancedCalculators:
    ...
    def calculate_fight_effectiveness(data: Dict[str, Any], hero_id: int) -> Dict[str, Any]:
        ...
        return {
            "damage_efficiency": round(min(100, (tf_damage / max(1, tf_duration)) * 5), 1) if tf_duration > 0 else 45.0,
            "kill_securing": round(min(1, solo_kill_ratio * 1.5), 2),
            "stun_follow_up": round(min(100, stun_impact * 20), 1),
            "ultimate_value": round(min(100, ability_chain_score * 8), 1),
    ...
    def calculate_advanced_positioning(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 10. Dangerous Positioning Deaths
        risky_deaths = sum(1 for d in deaths_log if (d.get("x", 128) + d.get("y", 128)) > 256)
        ...
        return {
            ...
            "gank_vulnerability": round(min(100, (avg_prox_dist / 10)), 1),
            ...
            "alone_vulnerability_score": round((alone_deaths / max(1, total_deaths)) * 10, 1) if total_deaths > 0 else 0,
    ...
    def calculate_decision_quality(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "timing_vs_avg": round(min(100, 50 + (mid_gpm / 10)), 1),
    ...
    def calculate_threat_prediction(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(lane_pos, dict):
            for zone, count in lane_pos.items():
                try:
                    c_val = sum(count.values()) if isinstance(count, dict) else count
                    total_pos_time += c_val
                    if int(zone) > 128: enemy_territory_time += c_val
                except: continue

        danger_exposure = (enemy_territory_time / max(1, total_pos_time)) * 100

        return {
            "gank_survival": round(vulnerability, 2),
    ...
    def calculate_stat_correlations(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "death_impact_cost": round(max(0, 100 - (data.get("deaths", 0) * 10)), 1),
